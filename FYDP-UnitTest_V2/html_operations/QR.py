...
from tkinter import filedialog, Toplevel, messagebox, Scrollbar, Listbox
from PIL import Image #ImageTk
from io import BytesIO
import segno
import webbrowser
import sys
#sys.path.append(r"C:\Users\Tiera\OneDrive\Desktop\FYDP-UnitTest_V2\FYDP-UnitTest_V2\database_operations\Database.py")
#sys.path.append(r"/home/user/FYDP-UnitTest_V2")
#sys.path.append(r"C:\Users\Tiera\OneDrive\Desktop\FYDP-UnitTest_V2\FYDP-UnitTest_V2")
import importlib.util
spec = importlib.util.spec_from_file_location("Database", r"/home/user/GetHub/FYDP-UnitTest_V2/database_operations/Database.py")
...
def generate_html_page(data, title):
    ...
    # Save the HTML
    #TODO: Replace asksaveasfilename with a predesignated Python write file call.
    # We don't want the museum staff to have to figure out where to put HTML files
    # remember that they have to be put in a specific folder.
...
#Laplante added table because SQL needs to know which table an entry must be deleted from
def confirm_delete(table, title, parent_window):
    ...
    tk.Button(
        ...
        padx=10,
        pady=5,
        command=lambda: [delete_entry(table, title), confirm_window.destroy(), parent_window.destroy()]  # Close both windows
...
def open_modify_delete_window(title):
    ...
    tk.Button(
        ...
        padx=10,
        pady=5,
        command=lambda: confirm_delete(table, title, modify_delete_window)
    ).pack(pady=10)
...
def fetch_data_for_title_dynamic(title):
    ...
    try:
        ...
        for table in tables:
            query = f"""
            SELECT title, description, id_num,
...
def create_folder(folder_name):
    ...
    command = (f"create table {folder_name} (title VARCHAR(75),description VARCHAR(3000),id_num VARCHAR(10),img_1 MEDIUMBLOB,"
